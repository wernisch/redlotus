...
<div class="group h-full flex flex-col rounded-xl overflow-hidden border border-[#ff0000]/40 bg-neutral-900/70
  ...
  <div class="relative bg-black aspect-[16/9]">
    <img class="absolute inset-0 w-full h-full object-contain" src="${icon}" alt="${game.name || "Game"}" loading="lazy">
    ...
    <div class="absolute bottom-0 left-0 p-4 pointer-events-none">
      ...
      <div class="flex items-center mt-1 text-sm gap-2">
        <span class="flex items-center text-green-400 font-bold">
          <svg class="w-6 h-6 mr-1 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          ...
          <span class="text-white"><span class="font-semibold">${(game.playing || 0).toLocaleString()}</span></span>
        </span>

        <span class="text-gray-400">|</span>

        <span class="flex items-center text-white font-semibold">
          <svg class="w-5 h-5 mr-1 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
